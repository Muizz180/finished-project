document.addEventListener('DOMContentLoaded', async () => {
    ...
    return;
      ...
      if (data.appointments && data.appointments.length > 0) {
       ...
       <button type="button" name="deleteAppointment" id="delete_btn"  value="${appointment.appointment_id}">Delete</button>       
        `;
...
document.querySelector('.appointmentsList').addEventListener('click', async (e) => {
  ...
  if (e.target.tagName === 'BUTTON' && e.target.name === 'deleteAppointment') {
    const appointmentId = e.target.value; // Get the appointment ID from the button
